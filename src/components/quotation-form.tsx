...
export default function QuotationForm({ data }: QuotationFormProps) {
  console.log("QuotationForm received data:", data);
  ...
  return (
    <div className="w-[21cm] min-h-[29.7cm] mx-auto p-8 bg-white text-black">
      ...
      <p className="text-sm mb-4 ml-8">
        Please quote your lowest net price, taxes included on the following
      ...
      <div className="space-y-1 mb-4">
        <p className="text-xs text-red-600">
          ***Please indicate your Brand Offer (if applicable) for each item (for
          evaluation purposes)***
        </p>
        <p className="text-xs text-red-600">
          ***Payment processing is ten (10) to fifteen (15) days after full
          delivery, inspection and acceptance***
        </p>
        <p className="text-xs text-red-600">
          ***Tax deductions applies for VAT and Non-VAT (2307)
        </p>
      </div>

      <p className="text-xs mb-6 text-justify">
        It is requested that you return this form in a sealed envelope by a
        messenger or mail it not later than ______________, 25___ at
        __________________. Only one canvass should be placed in an envelope
        marked "BID PROPOSAL". Brochure may be attached in the canvass form. A
        Purchase Order to the winning bidder shall be made immediately after the
        awarding. However, this office reserves the right to reject any offer
        without specified quality herein contained.
      </p>
      ...
      <div className="flex justify-between mt-10">
        <div>
          ...
          <p className="text-xs">
            Name, Position & Signature of Representative
          </p>
          <p className="text-xs mt-2">
            Date:{" "}
            <span className="border-b border-black inline-block w-32 ml-1"></span>
          </p>
        </div>
        ...
        <div className="text-right mr-10">
          ...
          <p className="text-xs font-bold">
            {data?.presidentName || "SEMA G. DILNA, EdD"},{" "}
            {data?.presidentTitle || "EdD"}
          </p>
          <p className="text-xs">
            {data?.presidentDesignation || "University President"}
          </p>
        </div>
      </div>
    </div>
