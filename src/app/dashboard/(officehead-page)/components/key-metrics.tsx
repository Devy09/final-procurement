...
export default function ProcurementDashboard() {
  ...
  useEffect(() => {
    async function fetchMetrics() {
      try {
        const response = await fetch(`/api/key-metrics/office-head`);
        if (!response.ok) {
  ...
  return (
    <div className="min-h-screen p-8 bg-background">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col bg-red-950 text-white p-4 rounded-lg w-full">
          <div className="flex items-center gap-2 text-2xl">
            ...
            <span className="font-bold">Office Head Dashboard</span>
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-4 lg:grid-cols-4">
        ...
        <StatsCard
          title="PPMP Items"
          value={metrics ? metrics.ppmpItemsCount.toString() : "Loading..."}
