...
export function DataTable({ data, setData }: DataTableProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("/api/procurement-officer-api/procurement-officer-ppmp", {
        method: "POST",
        headers: {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        const response = await fetch('/api/procurement-officer-api/procurement-officer-ppmp/preview', {
          method: 'POST',
          body: formData,
        });
  ...
  const handleUpload = async () => {
    try {
      ...
      const itemsToUpload = previewData.map(item => ({
        ppmp_item: item.ppmp_item,
        unit_cost: item.unit_cost,
        ppmp_category: 'Supplies' // Default category
      }));

      const uploadPromises = itemsToUpload.map(item => 
        fetch("/api/procurement-officer-api/procurement-officer-ppmp", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
